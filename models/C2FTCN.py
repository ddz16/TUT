...
class C2F_TCN(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.inc = inconv(configs.input_dim, 256)
        ...
        self.up = up(260, 128)
        # self.outcc0 = outconv(128, configs.num_classes)
        self.up0 = up(256, 128)
        # self.outcc1 = outconv(128, configs.num_classes)
        self.up1 = up(256, 128)
        self.outcc2 = outconv(128, configs.num_classes)
        self.up2 = up(384, 128)
        self.outcc3 = outconv(128, configs.num_classes)
        self.up3 = up(384, 128)
        self.outcc4 = outconv(128, configs.num_classes)
        self.up4 = up(384, 128)
        self.outcc = outconv(128, configs.num_classes)
    ...
    def forward(self, x, mask):
        ...
        prob_y5 = F.softmax(F.upsample(y5, size=y.shape[-1], mode='linear', align_corners=True), dim=1)
        prob_y4 = F.softmax(F.upsample(y4, size=y.shape[-1], mode='linear', align_corners=True), dim=1)
        prob_y3 = F.softmax(F.upsample(y3, size=y.shape[-1], mode='linear', align_corners=True), dim=1)
        ensemble_prob = (prob_y + prob_y5 + prob_y4 + prob_y3) / 4
