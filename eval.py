...
def func_eval(root, dataset, recog_path, file_list):
    ground_truth_path = root + dataset + "/groundTruth/"
    mapping_file = root + dataset + "/mapping.txt"
...
def main():
    ...
    if args.split == 0:
        for split in range(1, cnt_split_dict[args.dataset] + 1):
            ...
            file_list = args.root + args.dataset + "/splits/test.split{}".format(split) + ".bundle"
            acc, edit, f1s = func_eval(args.root, args.dataset, recog_path, file_list)
            acc_all += acc
            edit_all += edit
            f1s_all[0] += f1s[0]
            f1s_all[1] += f1s[1]
        ...
        recog_path = "./{}/".format(args.result_dir) + args.dataset + "/split_{}".format(split) + "/"
        file_list = args.root + args.dataset + "/splits/test.split{}".format(split) + ".bundle"
        acc_all, edit_all, f1s_all = func_eval(args.root, args.dataset, recog_path, file_list)
