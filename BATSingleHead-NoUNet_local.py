...
class LocalAttention(nn.Module):
    ...
    def forward(self, q, k, v, input_mask):
        ...
        if L > self.l_seg:  # and B != 1:
            # padding for the last segment
            n_seg = L // self.l_seg
            if L % self.l_seg != 0:
                q = torch.cat([q, torch.zeros(B, self.d_model, self.l_seg - L % self.l_seg).cuda()], dim=-1)
                k = torch.cat([k, torch.zeros(B, self.d_model, self.l_seg - L % self.l_seg).cuda()], dim=-1)
                v = torch.cat([v, torch.zeros(B, self.d_model, self.l_seg - L % self.l_seg).cuda()], dim=-1)
                n_seg += 1

            # padding at the start and end
            k = torch.cat([torch.zeros(B, self.d_model, self.band_width // 2).cuda(), k,
                           torch.zeros(B, self.d_model, self.band_width // 2).cuda()], dim=-1)
            v = torch.cat([torch.zeros(B, self.d_model, self.band_width // 2).cuda(), v,
                           torch.zeros(B, self.d_model, self.band_width // 2).cuda()], dim=-1)

            # segment and multi-head for efficient calculating
            q_seg = rearrange(q, 'b (h d) (n l) -> (b n) h d l', n=n_seg, h=self.h)  # (B,D,N*l) -> (B*N,H,D,l)
            k_seg = k.unfold(2, (self.l_seg + 2 * (self.band_width // 2)), self.l_seg)  # (B,D,N,l_k)
            k_seg = rearrange(k_seg, 'b (h d) n l -> (b n) h d l', h=self.h)  # (B*N,H,D,l_k)
            v_seg = v.unfold(2, (self.l_seg + 2 * (self.band_width // 2)), self.l_seg)  # (B,D,N,l_k)
            v_seg = rearrange(v_seg, 'b (h d) n l -> (b n) h d l', h=self.h)  # (B*N,H,D,l_k)

            # window mask generation
            window_mask = create_window_mask(self.l_seg, self.band_width)  # (l,l_k)

            # key padding mask generation
            if L % self.l_seg != 0:
                mask = torch.cat([input_mask[:, 0, :].cpu(), torch.zeros(B, self.l_seg - L % self.l_seg)], dim=-1)
            else:
                mask = input_mask[:, 0, :].cpu()
            mask = torch.cat([torch.zeros(B, self.band_width // 2), mask, torch.zeros(B, self.band_width // 2)], dim=-1)
            mask = mask.unfold(1, (self.l_seg + 2 * (self.band_width // 2)), self.l_seg)  # (B, N, l_k)
            mask = mask.reshape(-1, 1, self.l_seg + 2 * (self.band_width // 2)).bool()  # (B*N, 1, l_k)

            # total padding generation
            total_mask = ~ (window_mask & mask)  # (B*N, l, l_k)

            if self.rpe:
                rpe_bias = self.rpe_embedding.compute_bias(self.l_seg, self.l_seg + 2 * (self.band_width // 2))
                out, attn = scalar_dot_attn(q_seg, k_seg, v_seg, self.band_width, total_mask.cuda(), pe=rpe_bias, attn_dropout=self.attn_dropout, BALoss=self.baloss)  # (B*N, D, l_q)
            else:
                out, attn = scalar_dot_attn(q_seg, k_seg, v_seg, self.band_width, total_mask.cuda(), attn_dropout=self.attn_dropout, BALoss=self.baloss)

            if attn is not None:
                attn = rearrange(attn, '(b n h) l w-> b h (n l) w', n=n_seg, h=self.h)
                attn = attn[..., :L, :]  # (B, H, L, window_size)

            # reshape output
            out = rearrange(out, '(b n) d l -> b d (n l)', n=n_seg)
            out = self.W_out(self.dropout(out[..., :L]))  # * input_mask[:, 0:1, :]  # (B, d_model, L) -> (B, d_v, L)
        else:
            ...
            total_mask = ~ (window_mask.cuda() & input_mask)  # (B, L, L)
